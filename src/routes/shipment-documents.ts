...
router.get('/checklist-types', async (req, res) => {
  try {
    const types = await db.query<any[]>('SELECT * FROM DocumentChecklist ORDER BY createdAt DESC');
...
router.post('/checklist-types', async (req, res) => {
  try {
    ...
    const [result] = await db.query<any[]>('SELECT * FROM DocumentChecklist WHERE id = ?', [id]);
...
router.get('/shipment/:id/checklist', async (req, res) => {
  try {
    ...
    const definitions = await db.query<any[]>('SELECT * FROM DocumentChecklist WHERE isActive = 1');
...
router.post('/shipment/:id/upload', upload.single('file'), async (req, res) => {
  try {
    ...
    const existing = await db.query<any[]>('SELECT id FROM ShipmentDocument WHERE shipmentId = ? AND checklistId = ?', [shipmentId, checklistId]);
