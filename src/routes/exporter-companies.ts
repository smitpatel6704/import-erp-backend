...
router.get('/', async (req, res) => {
  try {
    const companies = await db.query<any>('SELECT * FROM ExporterCompany WHERE isActive = 1 ORDER BY name ASC');
...
router.post('/', async (req, res) => {
  try {
    ...
    const id = createId();
    await db.execute(
      'INSERT INTO ExporterCompany (id, name, contactPerson, mobile, email, address, country) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [id, name, contactPerson || null, mobile || null, email || null, address || null, country || null]
    );
    
    const company = await db.query<any>('SELECT * FROM ExporterCompany WHERE id = ?', [id]);
