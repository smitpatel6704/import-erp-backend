...
router.get('/:id', async (req, res) => {
  const { id } = req.params;
  try {
    const companies = await db.query<any>('SELECT * FROM Company WHERE id = ?', [id]);
    ...
    company.shipments = await db.query<any>('SELECT * FROM Shipment WHERE companyId = ? ORDER BY createdAt DESC LIMIT 10', [id]);
    company.invoices = await db.query<any>('SELECT * FROM Invoice WHERE companyId = ? ORDER BY createdAt DESC LIMIT 10', [id]);
    company.products = await db.query<any>('SELECT * FROM Product WHERE companyId = ? ORDER BY createdAt DESC LIMIT 10', [id]);

    const shipmentsCount = await db.query<any>('SELECT COUNT(*) as c FROM Shipment WHERE companyId = ?', [id]);
    const invoicesCount = await db.query<any>('SELECT COUNT(*) as c FROM Invoice WHERE companyId = ?', [id]);
    const productsCount = await db.query<any>('SELECT COUNT(*) as c FROM Product WHERE companyId = ?', [id]);
    const transactionsCount = await db.query<any>('SELECT COUNT(*) as c FROM Transaction WHERE companyId = ?', [id]);
...
router.get('/', async (req, res) => {
  try {
    ...
    const countRows = await db.query<any>(`SELECT COUNT(*) as c FROM Company WHERE ${whereClause}`, params);
    ...
    const companies = await db.query<any>(
      `SELECT * FROM Company WHERE ${whereClause} ORDER BY ${allowedSort} ${allowedDir} LIMIT ? OFFSET ?`,
      [...params, limit, skip]
    );

    for (const c of companies) {
      const shipmentsCount = await db.query<any>('SELECT COUNT(*) as c FROM Shipment WHERE companyId = ?', [c.id]);
      const invoicesCount = await db.query<any>('SELECT COUNT(*) as c FROM Invoice WHERE companyId = ?', [c.id]);
      const productsCount = await db.query<any>('SELECT COUNT(*) as c FROM Product WHERE companyId = ?', [c.id]);
      c._count = {
        shipments: shipmentsCount[0].c,
        invoices: invoicesCount[0].c,
        products: productsCount[0].c
