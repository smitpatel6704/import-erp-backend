...
router.get('/:id', async (req, res) => {
  const { id } = req.params;
  try {
    const containers = await db.query<any[]>(`
      SELECT c.*, 
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    const updated = await db.query<any[]>('SELECT * FROM Container WHERE id = ?', [id]);
...
router.get('/', async (req, res) => {
  try {
    ...
    const containerSize = req.query.containerSize as string || '';
    ...
    const [rows, countRows] = await Promise.all([
      db.query<any[]>(query, params),
      db.query<any[]>(countQuery, countParams)
    ]);
...
router.post('/', async (req, res) => {
  try {
    ...
    const newContainer = await db.query<any[]>('SELECT * FROM Container WHERE id = ?', [id]);
